...
(function() {
    ...
    DA.ElementUtils = {
        ...
        isElemVisible: function(elem) {
            ...
            if (elemStyle.transform !== "none") {
                var elemBounding = elem.getBoundingClientRect();
                width = elemBounding.width;
                height = elemBounding.height
            } else {
                width = elem.offsetWidth;
                height = elem.offsetHeight
            }
            if (width === 0 || height === 0 && elem.firstElementChild || elemStyle.visibility === "hidden") {
                return false
            }
            var parent = elem;
...
(function() {
    ...
    (function() {
        ...
        DA.Decorators.noRepeatData = function(event, $elem, logger) {
    ...
    DA.Decorators.inViewport = function(event, $elem, logger, options) {
        var name = "inViewport";
        var dfd = $.Deferred();
...
(function() {
    ...
    DA.Validator = {
        ...
        isValidSetterKey: function(str) {
            return this.validSetterKeyRegexp.test(str)
        },
        ...
        validateSetters: function(setters, ruleSetName) {
            if (!Array.isArray(setters)) {
                throw new Error("DA: 'setters' in ruleSet '" + ruleSetName + "' must be an array")
            }
            for (var i = 0, len = setters.length; i < len; i++) {
                this.validateSetter(setters[i], ruleSetName)
            }
        },
        validateSetter: function(setter, ruleSetName) {
            var key;
            var objsInSetter = 0;
            for (key in setter) {
                if (++objsInSetter > 1) {
                    throw new Error("DA: More than 1 key in setter in ruleSet '" + ruleSetName + "'")
                }
            }
            if (!setter.hasOwnProperty(key)) {
                throw new Error("DA: setter in ruleSet '" + ruleSetName + "' does not have any keys of its own")
            }
            if (!this.isValidSetterKey(key)) {
                throw new Error("DA: Invalid setter key '" + key + "' in ruleSet '" + ruleSetName + "'")
            }
        }
    };
...
(function() {
    ...
    DA.Controller = {
        ...
        listenToViewportTriggerEvents: function() {
            ...
            this.logic.viewportTriggerEvents.forEach(function(eventName) {
                $(document).on(eventName, function() {
                    that.logger.info("Saw viewportTriggerEvent: " + eventName);
        ...
        resolveCookie: function(aspect) {
            var val;
            if (aws_sc.CookieConsent.hasConsent(aspect.name)) {
                val = $.cookie(aspect.name)
            }
        ...
        resolveCSSAspect: function(aspect) {
            var $elem = $(aspect.selector);
            var val;
            if ($elem.length) {
                val = $elem.attr(aspect.attribute)
            } else {
                val = aspect.defaultValue
            }
            if (typeof val === "undefined" || val === "") {
        ...
        setupCustomEvent: function(event) {
            ...
            if (!AWS.DA.EventUtils.setElementEventRelationship($("body"), event)) {
                $(document).on(event.customEventName, function(e) {
        ...
        runConditionChain: function(event, $elem, cb) {
            this.logger.debug("Test conditions");
            if (typeof event.decorators !== "object") {
                event.decorators = []
            }
            var delegateCondition = function(name, event, $elem, logger, options) {
                logger.info("Test condition: " + name);
                return AWS.DA.Decorators[name](event, $elem, logger, options)
            };
            var dfds = [];
            for (var i = 0, len = event.decorators.length; i < len; i++) {
                dfds[dfds.length] = delegateCondition(event.decorators[i], event, $elem, this.logger, event.options)
            }
        ...
        createCustomFunction: function(fct) {
            if (typeof AWS.DA.fct !== "object") {
                AWS.DA.fct = {}
            }
            var that = this;
            AWS.DA.fct[fct.name] = function($elem) {
                if (typeof $elem !== "undefined" && $elem instanceof jQuery && $elem.length) {
                    this.logger.info("Execute function: " + JSON.stringify({
                        name: fct.name,
                        ruleSetKey: fct.ruleSet.key,
                        elem: $elem[0].toString(),
                        className: $elem[0].className
                    }));
                    var ruleSet = $.extend(true, {}, fct.ruleSet);
                    ruleSet.setters = that.preprocessSetters(ruleSet.setters, $elem);
                    that.processRuleSet(ruleSet)
                } else {
                    this.logger.error("Invalid or empty jQuery element passed to function: " + fct.name)
                }
            }
        },
        setClickHandler: function($elem, notSelector, explicitUrlAttr, cb) {
            ...
            var processClick = function($closestLink, attrName) {
            ...
            $elem.on("click.aws-da, mouseup.aws-da", function(e) {
        ...
        callLinkTrack: function(obj, callType, linkName) {
            ...
            this.setSVars(obj);
            var linkTrackVars = window[this.trackerVarName].defaultLinkVars + ",";
            for (var key in obj) {
                if (obj.hasOwnProperty(key)) {
        ...
        watchScroll: function() {
            ...
            AWS.DA._DA.debounceWindowEvent(function() {
    ...
    if (typeof AWSMA !== "undefined") {
        AWSMA.ready(function() {
